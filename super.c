...
static int parse_options(struct super_block *sb, char *options)
{
	...
	substring_t args[MAX_OPT_ARGS];  //存放挂载选项的挂载值
	char *p, *name;
	int arg = 0;
	kuid_t uid;
	kgid_t gid;
	...
	*/
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		/* f2fs_tokens是一个挂载选项字典（可在字典增加挂载选项），查询参数p是否与字典匹配 
			返回值args:该挂载选项p对应的挂载值
			（例如：background_gc=2，则p指的是整个字符串“background_gc=2”，args指的是“2”）
		*/
		token = match_token(p, f2fs_tokens, args);
		// printk(KERN_ERR"ZN trap: token=%d p=%s",token,p);
		// printk(KERN_INFO"ZN trap: token=%d p=%s",token,p);
		// f2fs_msg(sb, KERN_ERR, "ZN trap: token=%d p=%s",token,p);
		/*
...
static void f2fs_put_super(struct super_block *sb)
{
	 ...
	 */
	 /* 超级块为脏或者卸载之前未进行CP，则执行CP过程；其他情况不需要进行CP */
...
int f2fs_commit_super(struct f2fs_sb_info *sbi, bool recover)
{
	...
	/* ZN:
		先读取备份的super block，然后才读取当前的superb block。
		并且是先读入buffer_head，然后才调用__sync_dirty_buffer
	*/
...
static int f2fs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	/* start */
	/* 增加nvm_super_block结构 */
	struct nvm_super_block *nsb=NULL;
	/* 增加nvm_sb_info内存结构 */
	struct nvm_sb_info *nsbi;
	/* 增加nvm设备的block_device */
	struct block_device *nbdev;
	fmode_t mode = FMODE_READ | FMODE_WRITE;

	sector_t nsize;  //NVM容量（sector为单位）
	__le32 sectorsize;
	__le32 blocksize;
	__le32 blockcount;
	__le32 segmentsize;

	unsigned int mpt_ssd_count;
	unsigned int mpt_nvm_count;

	int nvm_err = -EINVAL;  //作为读NVM超级块的参数
	int nvm_recovery = 0;
...
try_onemore:
	...
	*/
	...
	/* 从SSD上读取f2fs超级块f2fs_super_block，
	...
	*/
	 ...
	 * 解析挂载参数
	 * 解析挂载选项之前，首先判断fsb中是否已经设置ndev_path：
	 * YES：表明之前已经关联NVM设备，则无需额外解析挂载选项,但是仍然需要解析常规选项
	 * NO：解析是否含有ndev_path挂载选项：
	*/
	...
	if(nsbi->nvm_flag & NVM_FIRST_MOUNR) {
		...
		/* 初始化nvm_super_block的字段 */
		memcpy(nsb->uuid, raw_super->uuid, sizeof(raw_super->uuid));
		nsb->mpt_blkaddr = raw_super->main_blkaddr;

		//NVM设备的总容量(扇区为单位),待测试，不一定正确

		nsize = nbdev->bd_part->nr_sects;
		...
		/* 计算MPT表大小(即SSD和NVM main区域segment总数) */
		mpt_ssd_count = raw_super->segment_count_main;
		mpt_nvm_count = nsb->main_segment_nums;
		nsb->mpt_entries = mpt_nvm_count + mpt_ssd_count;

		/* 计算lfu计数数组占用的物理块地址，共两份 */
		nsb->lfu_blkaddr0 = 2;
		nsb->lfu_blkaddr1 = 2 + (segmentsize - 2) / 2;
//		nvm_debug(NVM_ERR,"nsb->lfu_blkaddr0:%d", nsb->lfu_blkaddr0);
//		nvm_debug(NVM_ERR,"nsb->lfu_blkaddr1:%d", nsb->lfu_blkaddr1);


		/* MPT版本位图的大小，，MPT也有自己的位图，一位对应一个全是mpt_entry的块*/
		nsb->mpt_ver_map_bits = (nsb->mpt_entries* sizeof(unsigned int) - 1)/PAGE_SIZE + 1;

		nsb->map[0] = '0';	//用于记录位图的起始位置
		
		/* 初始化nvm_sb_info的字段 */
		res = init_nvm_sb_info(sbi, nsb);  //使用nsb初始化nsbi相关字段
		nvm_assert(!res);
		//! 2021年12月21日读到这
		/* 初始化NVM设备布局 */
		...
		nvm_flush_nsb_pages(sbi,0);	  //将cache中的nsb刷回mapping
//		nvm_flush_lfu_pages(sbi,0);	  //将cache中的lfu刷回mapping
		
		/*TODO:将META区域脏页刷回NVM*/
		
//		f2fs_sync_meta_pages(sbi, META, LONG_MAX, FS_CP_META_IO);
//		flush_meta_page_to_NVM(sbi);  //将META区域的所有页刷回NVM(包括nsb)
		...
		/* 读取MPT映射表信息,保存到MPT cache */
		f2fs_get_mpt(sbi);

		/* 读取LFU信息,保存到LFU cache */
//		f2fs_get_lfu(sbi);
	}
...
skip_recovery:
	...
	if (test_opt(sbi, BG_GC) && !f2fs_readonly(sb)) {
		...
		/*start*/
		//TODO:开启nvm GC线程
//		err = start_cold_to_ssd_thread(sbi);
//		if (err)
//			goto free_meta;
		//TODO:开启io队列统计线程
		err = init_aqu_sz_thread(sbi);
		if (err)
			goto free_meta;
		//TODO:开启热数据转移
		err = start_data_move_thread(sbi);
		if (err)
			goto free_meta;
		/*end*/
	}
...
{
	struct nvm_sb_info *nsbi = F2FS_SB(sb)->nsbi;
	if (sb->s_root) {
		set_sbi_flag(F2FS_SB(sb), SBI_IS_CLOSE);
		f2fs_stop_gc_thread(F2FS_SB(sb));
		//COMPLETED:关闭aqu_sz统计线程
		/*start*/
		if (nsbi->aqu_sz_task) {
			kthread_stop(nsbi->aqu_sz_task);
			nsbi->aqu_sz_task = NULL;
		}
		/*end*/
		//COMPLETED:关闭uphot_task热数据转移线程
		/*start*/
		if (nsbi->data_move_task) {
			kthread_stop(nsbi->data_move_task);
			nsbi->data_move_task = NULL;
		}
		/*end*/
