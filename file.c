...
static ssize_t f2fs_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	struct inode *inode = file_inode(file);
	ssize_t ret;
	printk(KERN_INFO"ZN trap: f2fs_file_write_iter");
	printk(KERN_INFO"ZN trap: iocb->ki_flags = %d",iocb->ki_flags);
	printk(KERN_INFO"ZN trap: inode->i_flags = %d",inode->i_flags);
	if (unlikely(f2fs_cp_error(F2FS_I_SB(inode))))
		return -EIO;
	...
	char string[32];
	copy_from_user(string, from->iov->iov_base, iov_iter_count(from));
	string[iov_iter_count(from)] = '\0';
	printk(KERN_INFO"ZN trap: content = %s",string);
	if (ret > 0) {
		...
		if ((iocb->ki_flags & IOCB_NOWAIT) &&
			...
			printk(KERN_INFO"ZN trap: f2fs_preallocate_blocks");
			err = f2fs_preallocate_blocks(iocb, from);	//ZN:进行预处理
			if (err) {
		...
		/* ZN：在这个函数里主要调用generic_perform_write */
	...
	printk(KERN_INFO"ZN trap: f2fs_file_write_iter end");
	return ret;
}
