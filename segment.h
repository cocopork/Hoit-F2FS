		...
		unsigned int segno)
{
	...
	//设置lfu计数为0
	atomic_set(&sbi->nsbi->lfu_count[sbi->nsbi->now_lfu_counter][segno], 0);
}
		...
		unsigned int segno)
{
	...
	if(get_map_flag(sbi->nsbi,nr_nvm_main_segs + ssd_main_segoff)){
		...
		unset_mapping(sbi, ssd_main_segoff);
		//段被回收释放，段计数需要清空
		atomic_set(&sbi->nsbi->lfu_count[sbi->nsbi->now_lfu_counter][ssd_main_segoff], -1);
		nvm_assert(get_map_flag(sbi->nsbi, nr_nvm_main_segs + ssd_main_segoff) == 0);
	}
	spin_unlock(&free_i->segmap_lock);
}
