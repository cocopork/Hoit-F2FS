...
void f2fs_balance_fs(struct f2fs_sb_info *sbi, bool need)
{
	...
	/*start*/
	//TODO:如果NVM设备空闲段少，则触发NVM设备GC
//	if (has_not_enough_free_nvm_secs(sbi->nsbi)) {
//        mutex_lock(&sbi->gc_mutex);
//		cold_to_ssd(sbi, false, false, NULL_SEGNO);
				...
				struct block_device *bdev)
{
	...
	//TODO:其他地方重定向bio
	/*start*/
    nvm_debug(NVM_DEBUG,"ERR:other positions redirect bio,bi_vcnt(%d)",bio->bi_vcnt);
    /*end*/
						...
						struct discard_cmd *dc)
{
	...
	if (!dc->error) {
		...
		if (bio) {
			...
			bio->bi_opf |= flag;
            //TODO:其他地方重定向bio
            nvm_debug(NVM_DEBUG,"ERR:other positions redirect bio!");
			submit_bio(bio);
...
static void update_sit_entry(struct f2fs_sb_info *sbi, block_t blkaddr, int del)
{
	...
	if (del > 0) {
//		nvm_debug(NVM_INFO, "set_cur_valid_map,ssd_segoff=%d,blk_no=%d", segno,
//				  sbi->raw_super->main_blkaddr + segno * sbi->blocks_per_seg + offset);
...
//		nvm_debug(NVM_INFO, "clear_cur_valid_map,ssd_segoff=%d,blk_no=%d", segno,
//				  sbi->raw_super->main_blkaddr + segno * sbi->blocks_per_seg + offset);
...
static void new_curseg(struct f2fs_sb_info *sbi, int type, bool new_sec)
{
	...
	/*start*/
	//COMPLETED:从NVM分配segment并且建立映射关系
	if (nvm_alloc(sbi, type)) {
		//TODO:分配失败时相关处理
		nvm_allocate_segment(sbi, curseg->segno,1);
	}
	/*end*/
}
...
static void change_curseg(struct f2fs_sb_info *sbi, int type)
{
	...
	nvm_debug(NVM_DEBUG,"SSR:change_curseg happened");
...
static int get_ssr_segment(struct f2fs_sb_info *sbi, int type)
{
	...
	if (v_ops->get_victim(sbi, &segno, BG_GC, type, SSR)) {// get_victim_by_default
		curseg->next_segno = segno;
		return 1;
	}
		...
		struct f2fs_io_info *fio, bool add_list)
{
	...
	*new_blkaddr = NEXT_FREE_BLKADDR(sbi, curseg);
	...
	old_seg = GET_SEGNO(sbi, old_blkaddr);
	if (old_seg != NULL_SEGNO){
		//如果旧块存在，需要转移旧块热度到新段中，然后再更新旧段sit信息
		/*start*/
		//得到旧段平均热度
//		count = calc_count(sbi, old_seg);
//		valid_blocks = get_valid_blocks(sbi, old_seg, false);
//		if (valid_blocks > 0 && count >= valid_blocks) {
////			printk(KERN_ERR "move hot count,count=%d,segno=%d", count / valid_blocks, curseg->segno);
//			atomic_add(count / valid_blocks, &sbi->nsbi->lfu_count[sbi->nsbi->now_lfu_counter][curseg->segno]);
//		}
		/*end*/
					...
					enum iostat_type io_type)
{
    ...
    //TODO:可能MPT区域的META数据
	if (unlikely(page->index >= MAIN_BLKADDR(sbi)+MAIN_OFFSET*sbi->blocks_per_seg))
				...
				bool recover_curseg, bool recover_newaddr)
{
	...
	nvm_debug(NVM_INFO, "ERR:f2fs_do_replace_block,old_blk=%d,new_blk=%d", old_blkaddr, new_blkaddr);
