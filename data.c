...
static inline void __submit_bio(struct f2fs_sb_info *sbi,
	...
	if (!is_read_io(bio_op(bio))) {
		...
		/* 
			ZN:bio->bi_iter.bi_size bio剩余还需要处理的数据量
			不过这里又除又余的不知道是什么意思，dummy pages是
			为了对齐而设置的虚拟数据。
		 */
	...
	//COMPLETED:提交bio之前按需进行设备信息和回写地址的更改
	/*start*/
...
//COMPLETED:为NVM相关代码调用__submit_bio开放接口
...
int f2fs_submit_page_bio(struct f2fs_io_info *fio) {
	...
	//TODO:如果fio->is_meta=false，但是type=META表明这个io请求是借用META的mapping读取数据，实际并不是META区域数据
	//对于这种情况，传递给__submit_bio一个NR_PAGE_TYPE,避免被重定向
	/*start*/
	if (!fio->is_meta && fio->type == META) {
		//不是meta数据，只是借用meta区域mapping
		__submit_bio(fio->sbi, bio, NR_PAGE_TYPE);
	} else {
...
void f2fs_submit_page_write(struct f2fs_io_info *fio) {
	...
	/* 合并写bio：如果旧bio的最后一个块地址，不与新来的块连续，则提交到目前为止已经合并的bio */
	if (io->bio && (io->last_block_in_bio != fio->new_blkaddr - 1 ||
					(io->fio.op != fio->op || io->fio.op_flags != fio->op_flags) ||
					!__same_bdev(sbi, fio->new_blkaddr, io->bio)))
		__submit_merged_bio(io);
	//TODO:bio跨段时候截断,提交原bio并重新建立bio
	/*start*/
	if (btype != META && io->bio) {
		...
		if (old_segno != new_segno) {
			nvm_debug(NVM_DEBUG, "BIO jump,old_segoff=%d,new_segoff=%d#", old_segno - main_segno,
					  new_segno - main_segno);
			__submit_merged_bio(io);
		}
	}
	/*end*/
	...
	/* 将新提交的page与原有的bio合并，如果合并之后，bio小于一个page，则马上提交 */
...
int f2fs_preallocate_blocks(struct kiocb *iocb, struct iov_iter *from) {
	...
	/* 
		ZN：F2FS支持数据直接保存在inode中，即inline data，
		大小约为3.4KB，保存在f2fs_inode->i_addr数组所在的
		空间中。
	 */
 ...
 * 将inode文件映射到内存中，把文件本身当作交换分区处理。
 * 由此可以看出，使用内存映射文件处理存储于磁盘上的文件时，将不必再对文件执行I/O操作，这意味着在对文件进行处理时将不必再为文件申请并分配缓存，
 * 所有的文件缓存操作均由系统直接管理，由于取消了将文件数据加载到内存、数据从内存到文件的回写以及释放内存块等步骤，
...
int f2fs_map_blocks(struct inode *inode, struct f2fs_map_blocks *map,
	...
	/* ZN：如果写入数据不超过一块，好像就不需要f2fs_map_blocks */
	if (!maxblocks)
		return 0;
	...
	//ZN：初始化dnode，dnode的作用是根据逻辑地址找物理地址
	set_new_dnode(&dn, inode, NULL, NULL, 0);
	...
	if (!is_valid_blkaddr(blkaddr)) {// 如果是无效的块地址（NEW_ADDR or NULL_ADDR），则新分配一个数据块
	...
	if (pgofs >= end)// 映射区域建立完成
		...
		goto next_block;// 处理dnode中的下一个数据块
...
//        nvm_debug(NVM_ERR, "Direct IO");
		map_bh(bh, inode->i_sb, map.m_pblk);// 为buffer_head结构赋予物理磁盘块地址

		bh->b_state = (bh->b_state & ~F2FS_MAP_FLAGS) | map.m_flags;
		bh->b_size = (u64) map.m_len << inode->i_blkbits;
		///TODO: 在文件直接IO过程中，对buffer_head结构做重定向
		/* start */
//		nvm_debug(NVM_INFO, "direct,i_opflags=%d", test_bit(IOP_WRITE, (volatile unsigned long *) &inode->i_opflags));
		nvm_redirect_dio(F2FS_I_SB(inode), bh, map.m_len, test_bit(IOP_WRITE, (volatile unsigned long *) &inode->i_opflags));
		//TODO:利用inode的i_opflags来标志数据的读写:写是1，读是0;用完清除标记
		clear_bit(IOP_WRITE, (volatile unsigned long *) &inode->i_opflags);
//		nvm_debug(NVM_INFO, "direct clear bit,i_opflags=%d", inode->i_opflags);
...
static int f2fs_mpage_readpages(struct address_space *mapping,
	...
	for (; nr_pages; nr_pages--) {
		...
		if (bio) {
			...
			if (old_segno != new_segno) {
				nvm_debug(NVM_DEBUG, "read bio jump,old_segoff=%d,new_segoff=%d#", old_segno - main_segno,
						  new_segno - main_segno);
...
static ssize_t f2fs_direct_IO(struct kiocb *iocb, struct iov_iter *iter) {
	...
	err = check_direct_IO(inode, iter, offset);// 检查块对齐情况，起始位置和长度都需要进行块对齐
	if (err)
		return err;
	...
	// 直接IO期间，对inode加GC信号量，防止在直接IO期间对该inode做GC
	...
	//TODO:利用inode的i_opflags来标志数据的读写:写是1，读是0
	/*start*/
//	nvm_debug(NVM_INFO, "direct before set,i_opflags=%d", inode->i_opflags);
	if (rw) {
		set_bit(IOP_WRITE, (volatile unsigned long *) &inode->i_opflags);
	}
	/*end*/
